function [ indicator_GT_ext ] = extrapolateGT( cdata, matchList, GTList, thresh_dist_GT )
% Make an extrapolated GT for evaluation
% output: 1 x nInitialMatches indicator vector
% view = cdata.view; matchList = cell2mat({cdata.matchInfo.match}'); GTList =  cdata.GT; thresh_dist_GT= extrapolate_thres
if length(view) == 1
...
if isempty(GTList)
    ...
    XY_view1= cdata.view(1,1).feat(matchList(:,1),1:2);
    XY_view2= cdata.view(1,2).feat(matchList(:,2),1:2); 
    %matchList_GT = matchList(find(GT),:);
    nGTList = size(GTList,1);
    indicator_GT_ext = zeros(1,size(matchList,1));
    
    for i=1:nGTList
        query_view1 = cdata.view(1,1).feat(GTList(i,1),1:2);
        query_view2 = cdata.view(1,2).feat(GTList(i,2),1:2); 
        if ~bSelf
